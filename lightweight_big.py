...
# Dynamically identify modules to apply LoRA
target_modules = set()
...
# Remove unsupported layers for LoRA
...
for param in model.parameters():
    if param.dtype in [torch.float16, torch.float32, torch.float64, torch.bfloat16]:  # Apply only to floating-point types
...
# TrainingArguments with memory optimization
...
# Custom Trainer for memory tracking
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Log runtime and memory usage to WandB
        end_time = time.time()
